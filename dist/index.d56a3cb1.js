...
async function dataTwo(id) {
    ...
    try {
        const response = await _axiosDefault.default.get(`https://restcountries.com/v2/name/${id}`);
        //log data in console
    ...
    //catch error
        ...
        if (err.response.status === 500) errorMessage.textContent = "Internal Server Error | 500";
    }
}
//function to get languages
function speak(lang) {
    let language = 'They speak ';
    if (lang.languages.length === 2) return language + `${lang.languages[0].name} and ${lang.languages[1].name}`;
    else if (lang.languages.length === 3) return language + `${lang.languages[0].name} and ${lang.languages[1].name} and ${lang.languages[2].name}`;
    else return language + `${lang.languages[0].name}`;
}
//function to get valuta and capital
function valutaCountry(val) {
    let cur = `The capital is ${val.capital} and you can pay with `;
    if (val.currencies.length === 2) return cur + `${val.currencies[0].name} and ${val.currencies[1].name}`;
    else return cur + `${val.currencies[0].name}`;
}
